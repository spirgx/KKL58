Public Class frmSeasonView

    Dim QueryStr As String
    Dim QuerySet As System.Data.DataSet
    Dim QueryAdapt As System.Data.SqlClient.SqlDataAdapter

    Private Sub frmSeasonView_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
    ...
    Private Sub ButtonQuery_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles ButtonQuery.Click
        ...
        QueryStr = QueryStr & " WHERE Year(TimeStamp) = '" & StartDatePicker.Value.Year.ToString & "' AND "

        Select Case ComboSeason.Text.Trim
            Case "第一季"
                QueryStr = QueryStr & "(Month(TimeStamp) = '1' OR Month(TimeStamp) = '2' OR Month(TimeStamp) = '3') "
            Case "第二季"
                QueryStr = QueryStr & "(Month(TimeStamp) = '4' OR Month(TimeStamp) = '5' OR Month(TimeStamp) = '6') "
            Case "第三季"
                QueryStr = QueryStr & "(Month(TimeStamp) = '7' OR Month(TimeStamp) = '8' OR Month(TimeStamp) = '9') "
            Case "第四季"
                QueryStr = QueryStr & "(Month(TimeStamp) = '10' OR Month(TimeStamp) = '11' OR Month(TimeStamp) = '12') "
        End Select

        QueryStr = QueryStr & ") t GROUP BY Y, M, D, CONVERT(varchar(10), TimeStamp, 120) Order by CONVERT(varchar(10), TimeStamp, 120)"
    ...
    Private Sub ButtonPrint_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles ButtonPrint.Click
        ...
        Dim SeasonViewReport As New frmSeasonViewReport

        SeasonViewReport.MdiParent = frmMain

        SeasonViewReport.FormBorderStyle = Windows.Forms.FormBorderStyle.Fixed3D
        SeasonViewReport.WindowState = FormWindowState.Normal
        ...
        paras(0) = New Microsoft.Reporting.WinForms.ReportParameter("Title", "金門酒廠" & AreaName & "廢水監測系統季報表")
        paras(1) = New Microsoft.Reporting.WinForms.ReportParameter("StartDate", Format(StartDatePicker.Value, "yyyy年").ToString)
        paras(2) = New Microsoft.Reporting.WinForms.ReportParameter("Season", ComboSeason.Text.Trim)
        ...
        SeasonViewReport.ReportViewer.LocalReport.SetParameters(paras)

        SeasonViewReport.ReportViewer.LocalReport.DataSources(0).Value = QuerySet.Tables("ReportSet")
        SeasonViewReport.ReportViewer.RefreshReport()

        SeasonViewReport.Show()
